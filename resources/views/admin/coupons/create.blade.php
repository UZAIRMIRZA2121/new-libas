...
<div class="table-card" style="max-width: 600px;">
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST" style="padding: 1.5rem;">
        @csrf
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="code" required placeholder="e.g. SUMMER20" value="{{ old('code', 'FIRSTORDER15') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 6px; outline: none; text-transform: uppercase;">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="number" name="discount_percentage" required min="1" max="100" value="{{ old('discount_percentage', 15) }}" style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 6px; outline: none;">
            @error('discount_percentage') <span style="color: #ef4444; font-size: 0.75rem;">{{ $message }}</span> @enderror
        </div>

        <div style="margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
            <input type="checkbox" name="is_used" id="is_used" {{ old('is_used') ? 'checked' : '' }} style="width: 16px; height: 16px;">
            <label for="is_used" style="font-size: 0.875rem; font-weight: 500;">Mark as Used</label>
        </div>

        <button type="submit" style="background: var(--primary); color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 6px; font-weight: 500; cursor: pointer;">Save Coupon</button>
    </form>
</div>
@endsection
